...
def home_section():
    ...
    if role_info:
        ...
        if 'PTS' in actual_cols and 'AST' in actual_cols and 'TRB' in actual_cols:
            ...
            fig = px.scatter(results, x='Actual', y='Predicted', hover_data=['Player'], labels={'Actual': 'Actual Points', 'Predicted': 'Predicted Points'}, title='Actual vs Predicted Points', color='Predicted', color_continuous_scale='Viridis')
            st.plotly_chart(fig)
